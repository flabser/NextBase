package finplanning.page.actions
			...
			try{
				...
				try{
					...
					if(docResponses != null && !docResponses.isEmpty()){
						def respDoc = doc.getResponses()[0]
						if(respDoc != null){
							if(db.deleteDocument(respDoc.getID(), true)){
								deletedList << new _Tag("entry",respDoc.getViewText())
							}else{
								unDeletedList << new _Tag("entry",respDoc.getViewText())
							}
						}
					}
				}catch(_Exception e){
