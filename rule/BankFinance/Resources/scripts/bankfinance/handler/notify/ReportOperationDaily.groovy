package bankfinance.handler.notify
 ...
 <tr>
  ...
  <td style="border-bottom:2px solid #CCC;text-align:center;" width="150px">Применено к</td>
			...
			collection.getEntries().each {
				...
				String bank_account = ""
				String type_account_transaction = ""
				...
				if( doc.getDocumentForm().equals("accrual") ){
					...
					type_account_transaction = doc.getValueString("typeoperationstaff")
					...
					bank_account = doc.getValueString("bank_account")
					type_account_transaction = doc.getValueString("type_account_transaction")

					if(bank_account.length()>0){
						gdoc = cdb.getGlossaryDocument(Integer.parseInt(bank_account))
						bank_account = gdoc.getValueString("name")
					}
					...
					if(typeoperation == "in"){
						...
						recipient = doc.getValueString("recipient")
						if(recipient.length() > 0){
							gdoc = cdb.getGlossaryDocument(Integer.parseInt(recipient))
					...
					gdoc = cdb.getGlossaryDocument(Integer.parseInt(type_account_transaction))
					typeoperation = gdoc.getValueString("name")
				}
  ...
  <td valign="top" style="border-top:1px solid #CCC;border-right:1px solid #CCC;">$bank_account</td>
