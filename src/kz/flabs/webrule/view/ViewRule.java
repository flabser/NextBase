...
public class ViewRule extends Rule implements Const {	
		...
		try{
			...
			try{	
				...
				for(int i = 0; i<nodename.getLength();i++){
					...
					if (f.isOn){	
						String attr = "";
						if ((!f.attrValue.equals("")) && (!f.attrName.equals(""))) attr = f.attrName + "=\"" + f.attrValue + "\""; 
	...
	public String getRuleAsXML(String app){
		...
		"<hits>" + hits + "</hits>" +		
		"<rununderuser>" + runUnderUser.toXML() + "</rununderuser>" +
		"<query>" + queryRuleName + "</query>" +
		"<fields>" + staticXMLContent + "</fields></rule>";
		return xmlText;
