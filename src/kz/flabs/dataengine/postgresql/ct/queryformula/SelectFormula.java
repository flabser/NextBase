package kz.flabs.dataengine.postgresql.ct.queryformula;
...
public class SelectFormula implements ISelectFormula {
	...
	public String getCountCondition(Set <String> complexUserID, Set <Filter> filters) {
        ...
        stringBuilder.append("SELECT count(md.DOCID) as count FROM MAINDOCS md");
        stringBuilder.append(" WHERE ");
        stringBuilder.append(sysCond.replaceAll("maindocs.", "md."));
        if (!"".equalsIgnoreCase(cuID)) {
            stringBuilder.append(" exists(select 1 from READERS_MAINDOCS where md.DOCID = READERS_MAINDOCS.DOCID and READERS_MAINDOCS.USERNAME IN (");
            stringBuilder.append(cuID);
            stringBuilder.append(")) ");
        }
        stringBuilder.append(existCond);
        String sql = stringBuilder.toString();
        if(sql.trim().toLowerCase().endsWith("and"))
            sql = sql.trim().substring(0, sql.trim().length() - 3);
        return sql;
	}
