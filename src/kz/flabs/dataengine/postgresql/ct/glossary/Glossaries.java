package kz.flabs.dataengine.postgresql.ct.glossary;

import kz.flabs.appenv.AppEnv;
import kz.flabs.dataengine.*;
import kz.flabs.dataengine.h2.Database;
import kz.flabs.dataengine.h2.glossary.GlossariesTuner;
...
public class Glossaries extends DatabaseCore implements IGlossaries, Const{
    ...
    public int updateAccount(Glossary account) throws DocumentException {
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            String updateSQL = "UPDATE account\n" +
                    "   SET name=?, amountcontrol=?, owner=?, observers=?\n" +
                    " WHERE id=?\n";
            PreparedStatement pst = conn.prepareStatement(updateSQL);
            pst.setString(1, account.getValueAsString("name")[0]);
            pst.setInt(2, Integer.parseInt(account.getValueAsString("name")[0]));
            pst.setString(3, account.getValueAsString("user")[0]);
            Array observers = conn.createArrayOf("text", account.getValueAsString("observers"));
            pst.setArray(4, observers);
            pst.setInt(5, account.getDocID());
            pst.executeUpdate();
            conn.commit();
            pst.close();
        } catch (SQLException e) {
        ...
        return account.getDocID();
    }

    public int insertAccount(Glossary account) throws DocumentException {
        Connection conn = dbPool.getConnection();
        int key = 0;
        try {
            conn.setAutoCommit(false);
            String insertSQL = "INSERT INTO Account(\n" +
                    "    name, amountcontrol, owner, observers, type)\n" +
                    "    VALUES (?, ?, ?, ?, ?)";
            PreparedStatement pst = conn.prepareStatement(insertSQL);

            pst.setString(1, account.getValueAsString("name")[0]);
            pst.setInt(2, Integer.parseInt(account.getValueAsString("amountcontrol")[0]));
            pst.setString(3, account.getValueAsString("user")[0]);
            Array observers = conn.createArrayOf("varchar", account.getValueAsList("observers").toArray());
            pst.setArray(4, observers);
            pst.setInt(5, account.docType);
            pst.executeUpdate();
	...
	public int insertGlossaryDocument(Glossary doc) throws DocumentException {
        ...
        if (doc.docType == Const.DOCTYPE_ACCOUNT) {
            return insertAccount(doc);
        }
