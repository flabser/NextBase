package kz.flabs.dataengine.postgresql.ct;

import kz.flabs.appenv.AppEnv;
import kz.flabs.dataengine.*;
import kz.flabs.dataengine.h2.DBConnectionPool;
import kz.flabs.dataengine.postgresql.DDEScripts;
...
public class DatabaseDeployer implements IDatabaseDeployer, IProcessInitiator {
    ...
    public boolean deploy(){
        try {
            ...
            checkAndCreateTable(kz.flabs.dataengine.postgresql.ct.DDEScripts.getAccountDDE(), "ACCOUNT");
            checkAndCreateAuthorsReadersTbl("AUTHORS_ACCOUNT", "ACCOUNT", "id");
            checkAndCreateTable(kz.flabs.dataengine.postgresql.ct.DDEScripts.getActionDDE(), "ACTION");
            checkAndCreateAuthorsReadersTbl("AUTHORS_ACCOUNT", "ACTION", "id");
            checkAndCreateTable(kz.flabs.dataengine.postgresql.ct.DDEScripts.getCostCenterDDE(), "COSTCENTER");
    ...
    public boolean checkAndCreateTrigger(String mainTableName) {
        ...
        try {
            ...
            s.addBatch(kz.flabs.dataengine.postgresql.DDEScripts.getAttachmentTriggerDDE(mainTableName));
            s.executeBatch();
            s.close();
            conn.commit();
            return true;
    ...
    public boolean checkAndCreateResponseTrigger(String mainTableName) {
        ...
        try {
            ...
            s.addBatch(kz.flabs.dataengine.postgresql.DDEScripts.getResponsesTriggerDDE(mainTableName));
            s.executeBatch();
            s.close();
            conn.commit();
            return true;
    ...
    public void checkAndCreateAuthorsReadersTbl(String tableName, String mainTable, String docID) throws SQLException{
        ...
        try{
            if(!DatabaseUtil.hasTable(tableName, conn)){
                Statement s = conn.createStatement();
                if (!s.execute(kz.flabs.dataengine.postgresql.DDEScripts.getAuthorReadersDDE(tableName, mainTable, docID))){
