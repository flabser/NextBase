...
import kz.flabs.dataengine.postgresql.ct.filters.Filters;
import kz.flabs.dataengine.postgresql.ct.glossary.Glossaries;
import kz.flabs.dataengine.postgresql.ct.glossary.GlossaryQueryFormula;
import kz.flabs.dataengine.postgresql.ct.queryformula.*;
import kz.flabs.dataengine.postgresql.ct.structure.StructQueryFormula;
...
public class Database extends kz.flabs.dataengine.h2.Database implements IDatabase, Const {
                    ...
                    switch (field.getTypeAsDatabaseType()) {
                        ...
                        case DATETIMES:
                            ...
                            try {
                                ...
                                kz.flabs.dataengine.postgresql.Database.logger.errorLogEntry(e + ", field=" + field.name);
                                return -1;
                            }
                            pst.executeUpdate();
                            pst.close();
                        ...
                        case DATE:
                            if (field.valueAsDate == null) {
                                kz.flabs.dataengine.postgresql.Database.logger.errorLogEntry("Unable to convert \"null\" to date : " + field.name);
                                continue;
                            }
                            try {
                                pst = conn.prepareStatement("insert into CUSTOM_FIELDS(DOCID, NAME, VALUEASDATE, TYPE)"
            ...
            try {
                ...
                for (Field field : doc.fields()) {
                    ...
                    switch (field.getTypeAsDatabaseType()) {
                        ...
                        case DATETIMES:
                            if (field.valueAsDate == null) {
                                kz.flabs.dataengine.postgresql.Database.logger.errorLogEntry("Unable to convert empty date to DB format: " + field.name);
                                continue;
                            }
                            try {
    ...
    public ISelectFormula getSelectFormula(FormulaBlocks blocks) {
        switch (blocks.docType) {
            case STRUCTURE:
                return new StructSelectFormula(blocks);
            case GLOSSARY:
                return new GlossarySelectFormula(blocks);
            default:
                return new SelectFormula(blocks);
        }
    }
}
