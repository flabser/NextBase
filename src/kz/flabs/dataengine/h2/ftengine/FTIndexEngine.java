...
public class FTIndexEngine implements IFTIndexEngine, Const {
    ...
    public _ViewEntryCollection search(String keyWord, User user, int pageNum, int pageSize, String[] sorting, String[] filters) throws FTIndexEngineException {
        ...
        String fields = "mt.docid, mt.doctype, mt.ddbid, mt.form, mt.has_attachment, mt.viewtext, " + DatabaseUtil.getViewTextList("mt") + ", mt.viewnumber, mt.viewdate";
        ...
        try {
            ...
            if (pageNum == 0)pageNum = RuntimeObjUtil.countMaxPage(result.size(), pageSize);
                ...
                switch (table.toUpperCase()) {
                    case "CUSTOM_FIELDS":
                        sql = "SELECT distinct " + fields + " FROM maindocs as mt, readers_maindocs as rm, custom_fields as cf where exists " +
                                "(select * from readers_maindocs where readers_maindocs.docid = rm.docid and readers_maindocs.username in (" + cuID + ")) and exists " +
                                "(select * from custom_fields where custom_fields.docid = cf.docid and custom_fields.id = " + key + ")" +
                    ...
                    case "EXECUTIONS":
                        sql = "SELECT distinct " + fields + " FROM executions as mt, readers_executions as re where mt.docid = " + key + " and exists " +
                                "(select * from readers_executions where readers_executions.docid = re.docid and readers_executions.username in (" + cuID + "))" +
                                (filterCondition.length() != 0 ? " and " + filterCondition : "") +
                                getOrderCondition(sorting) + " " +
                                getPagingCondition(pageSize, pageNum) + ";";
                        break;
