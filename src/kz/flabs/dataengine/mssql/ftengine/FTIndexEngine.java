...
public class FTIndexEngine implements IFTIndexEngine, Const {
    ...
    public StringBuffer ftSearch(Set<String> complexUserID, String absoluteUserID, String keyWord, int offset, int pageSize) throws DocumentException, FTIndexEngineException {
        ...
        try {
            ...
            int key;
            String table;
/*            String sql = "SELECT distinct docid, 'maindocs' as tablename FROM maindocs where contains (viewtext, '" + _keyWord + "') or contains (viewtext1, '" + _keyWord + "') or contains (viewtext2, '" + _keyWord + "') or contains (viewtext3, '" + _keyWord + "') \n" +
            " and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT id as docid, 'custom_fields' as tablename FROM custom_fields where contains (value, '" + _keyWord + "') \n" +
            " and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"' and \n" +
            "  docid not in (SELECT distinct docid FROM maindocs where contains (viewtext, '" + _keyWord + "') or contains (viewtext1, '" + _keyWord + "') or contains (viewtext2, '" + _keyWord + "') or contains (viewtext3, '" + _keyWord + "')))" + 
            " union \n" +
            "SELECT distinct docid, 'tasks' as tablename FROM tasks where contains (viewtext, '" + _keyWord + "') or contains (viewtext1, '" + _keyWord + "') or contains (viewtext2, '" + _keyWord + "') or contains (viewtext3, '" + _keyWord + "') \n" +
            " and docid in (select docid from readers_tasks where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT distinct docid, 'executions' as tablename FROM executions where contains (viewtext, '" + _keyWord + "') or contains (viewtext1, '" + _keyWord + "') or contains (viewtext2, '" + _keyWord + "') or contains (viewtext3, '" + _keyWord + "') \n" +
            " and docid in (select docid from readers_executions where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT distinct docid, 'projects' as tablename FROM projects where contains (viewtext, '" + _keyWord + "') or contains (viewtext1, '" + _keyWord + "') or contains (viewtext2, '" + _keyWord + "') or contains (viewtext3, '" + _keyWord + "') \n" +
            ...
            String sql = " DECLARE @intStartRow int; " +
                    ...
                    " WITH blogs AS" +
                    " (SELECT  distinct docid, 'custom_fields' as tablename, " +
                    " ROW_NUMBER() OVER(ORDER BY ID " + Const.DEFAULT_SORT_ORDER + ") as intRow, " +
                    " COUNT(ID) OVER() AS intTotalHits " +
                    ...
                    " and docid in (select docid from readers_maindocs" + " where username = '" + absoluteUserID + "')" + " ) " +
                    " SELECT distinct docid, 'CUSTOM_FIELDS' as tablename FROM blogs" +
            ...
            for (String tableName : tables) {
                sql = " DECLARE @intStartRow int; " +
                        ...
                        " SET @intEndRow = @intPage * @intPageSize;" +
                        " WITH blogs AS" +
                        " (SELECT  distinct docid, viewdate, '" + tableName + "' as tablename, " +
                        ...
                        " FROM " + tableName + " where contains (viewtext, '" + _keyWord + "') " + containsViewtext + " \n" +
                        " and docid in (select docid from readers_maindocs where username = '" + absoluteUserID + "')" + " ) " +
    ...
    private void processResultSet(Connection conn, ResultSet rs, StringBuffer xmlContent, Set<String> complexUserID, String absoluteUserID) throws ComplexObjectException {
        Set<String> set = new HashSet<String>();
        try {
            while (rs.next()) {
                Document doc = null;
    ...
    public int ftSearchCount(Set<String> complexUserID, String absoluteUserID, String keyWord) throws DocumentException {
        ...
        try {
            ...
            String sql = "SELECT count(distinct docid) as count, 'maindocs' as tablename FROM maindocs where contains (viewtext, '" + _keyWord + "') " + containsViewtext + " \n" +
            " and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            ...
            "  docid not in (SELECT distinct docid FROM maindocs where contains (viewtext, '" + _keyWord + "') " + containsViewtext + " )" +
            " union \n" +
            "SELECT count(distinct docid) as count, 'tasks' as tablename FROM tasks where contains (viewtext, '" + _keyWord + "') " + containsViewtext + " \n" +
            " and docid in (select docid from readers_tasks where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT count(distinct docid) as count, 'executions' as tablename FROM executions where contains (viewtext, '" + _keyWord + "') " + containsViewtext + " \n" +
            " and docid in (select docid from readers_executions where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT count(distinct docid) as count, 'projects' as tablename FROM projects where contains (viewtext, '" + _keyWord + "') " + containsViewtext + "  \n" +
            " and docid in (select docid from readers_projects where username = '"+ absoluteUserID +"');";
    ...
    public int updateFTIndex() throws FTIndexEngineException {
        ...
        try {
            ...
            try{
                ...
                throw new FTIndexEngineException(FTIndexEngineExceptionType.QUERY_UNRECOGNIZED, "");
            }
            conn.commit();
            stmt.close();
        } catch (SQLException e) {
    ...
    @Override
    public _ViewEntryCollection search(String keyWord, User user, int pageNum, int pageSize, String[] sorting, String[] filters) throws FTIndexEngineException {
        return null;
    }
