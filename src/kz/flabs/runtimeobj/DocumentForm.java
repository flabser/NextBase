...
public class DocumentForm extends Form {
	...
	public DocumentForm(HashMap<String, String[]> fields, AppEnv env, FormRule rule, UserSession user) throws RuleException, QueryFormulaParserException {
		super(env, rule, user);
		this.fields = fields;
	}
	...
	public String getDefaultFieldsAsXML(int parentDocID, int parentDocType, int page, String lang) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{
		...
		if(rule.qoEnable){ 
			...
			qoResult = qop.processScript(rule.qoClassName);			
		}
		...
		"openfrompage=\"" + page + "\" status=\"new\" "  + 
		mode[0] + ">" + outlineContent + "<actions>" + actions + "</actions>" + elementsContent + "<fields>" + fieldsContent + "</fields>" + mode[1] + captions + "</document>";	
	}

	public String getFormAsXML(BaseDocument doc, int page, int parentDocID, int parentDocType, String lang) throws DocumentException, DocumentAccessException, RuleException, QueryFormulaParserException, ClassNotFoundException, QueryException, LocalizatorException, _Exception, ComplexObjectException {
		StringBuffer elementsContent = new StringBuffer(1000);
		StringBuffer fieldsContent = new StringBuffer(1000);
		@Deprecated
		String actions = "";	
	...
	public XMLResponse save(String key, HashMap<String, String[]> fields, int parentDocID, int parentDocType, int pageSize, String currentLang) throws DocumentAccessException, DocumentException, RuleException, QueryFormulaParserException, ClassNotFoundException, ComplexObjectException{
		...
		if(rule.advancedQSEnable){ 
			qsp = new QuerySaveProcessor(env, (BaseDocument)doc, userSession.currentUser,currentLang, fields);
			QuerySaveResult qsResult = qsp.processScript(rule.qsClassName);
		...
		if (result.resultFlag){
			...
			if (result.resultFlag){
				...
				if (rule.advancedPSEnable){
					if (doc instanceof Glossary){
						psp = new PostSaveProcessor((BaseDocument)doc, userSession.currentUser);
					}else{
						psp = new PostSaveProcessor((Document)doc, userSession.currentUser);
					}
			...
			if (redirectURL != null){		
				/*try{
					...
					redirectURL = redirectURL + "&amp;page=" + page[0];
				}catch(Exception nfe){
