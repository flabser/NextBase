	...
	public int fetch(int pageNum, int pageSize, int parentDocID, int parentDocType, Set<DocID> toExpandResp, Set<String> toExpandCategory, DocID toFlash, Map<String, String[]> fields) throws DocumentException, DocumentAccessException, QueryFormulaParserException{
		...
		case MACRO:
			if(rule.getQuery().getValue().equalsIgnoreCase("responses")){
				...
				xmlFragment.append(structAsXML);
				//}
			} else if (rule.getQuery().getValue().equalsIgnoreCase("workdoc_signer") || rule.getQuery().getValue().equalsIgnoreCase("signer") || rule.getQuery().getValue().equalsIgnoreCase("responsible_section") || rule.getQuery().getValue().equalsIgnoreCase("tech_engineer") || rule.getQuery().getValue().equalsIgnoreCase("tech_supervisor") || rule.getQuery().getValue().equalsIgnoreCase("operator") || rule.getQuery().getValue().equalsIgnoreCase("top_manager")) {
