...
public class Form extends Content implements Const{
    ...
    public String getDefaultFieldsAsXML(int parentDocID, int parentDocType, int page, String lang) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{
        ...
        return "<document isvalid=\"true\"  " +
                "parentdocid=\"" + parentDocID +"\" parentdoctype=\"" + parentDocType +"\" doctype=\"" + rule.docType + "\" " +
                "openfrompage=\"" + page + "\" status=\"new\" viewtext=\"\" " +
                mode[0] + ">" + outlineContent + "<actions>" + actions + "</actions><fields>" + xmlContent + "</fields>" + mode[1] + captions + "</document>";


    }

    public String getFormAsXML(BaseDocument doc, int page, int parentDocID, int parentDocType, String lang) throws DocumentException, DocumentAccessException, RuleException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{
    ...
    public String getFormAsXML(BaseDocument doc, String lang) throws DocumentException, DocumentAccessException, RuleException, QueryFormulaParserException, QueryException, ComplexObjectException{
    ...
    @Deprecated
    public XMLResponse save(String key, HashMap<String, String[]> fields, int parentDocID, int parentDocType, int pageSize, String currentLang) throws DocumentAccessException, DocumentException, RuleException, QueryFormulaParserException, ClassNotFoundException, ComplexObjectException{
            ...
            switch(rule.docType){
                ...
                case DOCTYPE_EXECUTION:
                    if (parentDocID == 0 || parentDocType == DOCTYPE_UNKNOWN){
                        ...
                        if (db.hasDocumentByComplexID(parentDocID, parentDocType)){
                            doc = db.getExecutions().getExecutionByID(docID, userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
                            if (doc.isNewDoc()){
                                doc.parentDocID = parentDocID;
                                doc.parentDocType = parentDocType;
                            }
                ...
                case DOCTYPE_GLOSSARY:
                    doc = db.getGlossaries().getGlossaryDocumentByID(docID, true, userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
                    if (parentDocID != 0 && parentDocType != DOCTYPE_UNKNOWN){
                        if (db.hasDocumentByComplexID(parentDocID, parentDocType)){
                            if (doc.isNewDoc()){
                                doc.parentDocID = parentDocID;
        ...
        if(rule.querySaveEnable){
            qsp = new QuerySaveProcessor(env, (BaseDocument)doc, userSession.currentUser,currentLang);
        ...
        if (result.resultFlag){
            ...
            if (result.resultFlag){
                ...
                if(rule.docType == DOCTYPE_GLOSSARY) CachePool.flush();
                ...
                if (rule.postSaveEnable){
                    PostSaveProcessor psp = new PostSaveProcessor((BaseDocument)doc, userSession.currentUser);
                    psp.setClass(rule.postSaveClass);
                    psp.start();
                }
            }
            if (doc.toDeleteAfterSave != null) Environment.fileToDelete.add(doc.toDeleteAfterSave);
        }else{
    ...
    private StringBuffer getDocumentAsXML(FormDataSourceSupplier fdss, SourceSupplier captionTextSupplier) throws DocumentException, DocumentAccessException, RuleException, QueryFormulaParserException, QueryException, ComplexObjectException{
        String fieldList = "";
        StringBuffer xmlText = new StringBuffer(1000);
        xmlText.append("<fields>");

        for(ShowField sf: rule.showFieldsMap.values()){
            String attrVal = "", caption = ""; fieldList = "";
    ...
    protected String[] getEditModeAttr(String lang) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException{
    ...
    protected String[] getEditModeAttr(BaseDocument doc, String lang) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException{
        String[] result = new String[2];
        if (env.globalSetting.edsSettings.isOn == RunMode.ON){
            switch(doc.editMode){
