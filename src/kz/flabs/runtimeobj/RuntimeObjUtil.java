...
public class RuntimeObjUtil implements Const {
	...
	public BaseDocument getGrandParentDocument(IDatabase db, BaseDocument doc) throws DocumentException, DocumentAccessException, ComplexObjectException{
		if (!doc.isNewDoc()){
	...
	public static Calendar[] getPeriodDates(_PeriodType pType, boolean sixWorkDays, HolidayCollection holidays){
        ...
        if(pType == _PeriodType.WORKDAYS){
            ...
            for (int i = Calendar.getInstance().getActualMaximum(Calendar.DAY_OF_YEAR); i > 0 ; i--) {
                ...
                if (!isHoliday && date.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY && (sixWorkDays || date.get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY)){
		...
		for (int i = Calendar.getInstance().getActualMaximum(Calendar.DAY_OF_YEAR); i > 0 ; i--) {
			...
			case END_OF_MONTH:
				...
				break;
			case END_OF_QUARTER:
				isEnd = date.get(Calendar.MONTH) == ((int)Math.floor(((float)date.get(Calendar.MONTH))/3.0)) * 3 + 2 &&
