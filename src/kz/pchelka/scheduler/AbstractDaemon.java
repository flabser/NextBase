...
public ArrayList<DaysOfWeek> daysOfWeek = new ArrayList<DaysOfWeek>();
	...
	@Override
	abstract public int process(IProcessInitiator processOwner) throws DocumentAccessException, RuleException,	QueryFormulaParserException, QueryException;
	...
	public void postSuccess(Calendar finishTime) {
		...
		Server.logger.normalLogEntry(getID() + "\" has been success finished. Next start " + Util.dateTimeFormat.format(finishTime.getTime()));
	...
	public String toXML() {
		...
		String xmlFragment = "<id>" + getID() + "</id>" +
				"<type>" + getDeamonType() + "</type>" +
				"<trigger>" + getTriggerType() + "</trigger>" +
				"<lastsuccess>" + lst + "</lastsuccess>" +
				"<nexttime>" + nt + "</nexttime>" +
				"<successrun>" + getSuccessRunCount() + "</successrun>" +
				"<runhistory>" + getSuccessRunHistory() + "</runhistory>" +
				"<status>" + stat + "</status>";
