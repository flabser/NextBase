					...
					<table style="width:auto; border-collapse:collapse">
						<tr  class="notviewtr">
							<td style="max-width:35px; width:35px;  padding-left:3px; text-align:center">
								<xsl:if test="/request/@id = 'workdocprj' or /request/@id = 'outdocprj'">
								...
								<xsl:choose>
									...
									<xsl:when test="/request/@id = 'tasks'  or /request/@id = 'mytasks' or /request/@id = 'taskforme'">
										<font style="font-size:9px;">
											<xsl:attribute name="color">
												<xsl:choose>
													<xsl:when test="dbd &lt; 0">#F44F00</xsl:when>
													<xsl:when test="dbd = 0">#FFD700</xsl:when>
									...
									<!-- <xsl:when test="dbd = 0">
										<img id="control" src="/SharedResources/img/classic/icons/bullet_orange.png" style="height:14px; width:14px">
											<xsl:attribute name="title"><xsl:value-of select="/request/columns/column[@id='CTRLDATE']/@caption"/> : <xsl:value-of select="dbd"/>&#xA0;<xsl:value-of select="/request/columns/column[@id='DAYS']/@caption"/> </xsl:attribute>
										</img>
									</xsl:when>
									<xsl:when test="dbd &lt; 0">
										<img id="control" src="/SharedResources/img/classic/icons/bullet_red.png" style="height:14px; width:14px ">
											<xsl:attribute name="title"><xsl:value-of select="/request/columns/column[@id='CTRLDATE']/@caption"/> : <xsl:value-of select="dbd"/>&#xA0;<xsl:value-of select="/request/columns/column[@id='DAYS']/@caption"/> </xsl:attribute>
										</img>
									</xsl:when>		 -->
								</xsl:choose>
							</td>
							<xsl:if test="/request/@id != 'workdocprj' and /request/@id != 'outdocprj' ">
								<td style="min-width:16px; padding-left:3px">
									<xsl:if test="allcontrol = 0 or /request/@id = 'completetask'">
								...
								<tr class="th">
									<xsl:choose>
										...
										<xsl:otherwise>
											...
											<td style="width:80px;  padding:3px" class="thcell"></td>
											<xsl:if test="/request/@id != 'tasks' and /request/@id != 'taskforme'  and /request/@id !='mytasks' and /request/@id !='completetask'">
