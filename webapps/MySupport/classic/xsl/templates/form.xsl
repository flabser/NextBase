...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="cssandjs">
        ...
        <script type="text/javascript" src="/SharedResources/jquery/js/ckeditor/ckeditor.js"></script>
		...
		<script>
			...
			removedcaption= '<xsl:value-of select="document/captions/removedcaption/@caption"/>';
            newmail= '<xsl:value-of select="document/captions/newmail/@caption"/>';
            topic= '<xsl:value-of select="document/captions/topic/@caption"/>';
            text= '<xsl:value-of select="document/captions/text/@caption"/>';
            blankmessage = '<xsl:value-of select="document/captions/blankmessage/@caption"/>';
            notSentMessage = '<xsl:value-of select="document/captions/notSentMessage/@caption"/>';
            sentMessage = '<xsl:value-of select="document/captions/sentMessage/@caption"/>';
		</script>
		<script type="text/javascript">    
			$(function() {
				$("#tabs").tabs();
	...
	<xsl:template name="documentheader">
		<div style="position:absolute;  top:0px; left:0px; width:100%; height:70px; border-bottom:1px solid rgba(190,213,224,0.89); background:url('classic/img/blue_background.jpg'); z-index:2">
 			<span style="float:left">
		     	<img src="/SharedResources/logos/mysupport.png" style="margin:5px 0px 0px 10px; height:60px"/>  
			</span>
	     	 
			<span style="float:left; margin:15px 0 0 0px">
				<font style="font-size:1.6em; color:#404040; padding-left: 10px">MySupport</font>
				...
				<xsl:for-each select="//response/content/outline">
					<div>
						...
						<div class="outlineEntry" id="{@id}">
							<script>
								if	($.cookie("MySupport_<xsl:value-of select='@id'/>") != 'null'){
									$("#<xsl:value-of select='@id'/>").css("display",$.cookie("MySupport_<xsl:value-of select='@id'/>"))
									if($.cookie("MySupport_<xsl:value-of select='@id'/>") == "none"){
