...
function uploadComplete(tableID, doc) {
	...
	if (st = 'ok'){
		...
		if(d.indexOf(".") != -1){
			...
			"<a href='javascript:deleterow(&quot;"+sesid +"&quot;,&quot;"+ d +"&quot;,&quot;"+ csf +"&quot;)'><img src='/SharedResources/img/iconset/cross.png' style='margin-left:5px; width:10px; height:10px; vertical-align:6px'  title='"+delete_file+"'/></a>" +
			"</td><td></td</tr>");
...
function EmailToManager(){

    var divhtml = '<div id="dialog-email" title="'+newmail+'" >'
    divhtml += '<div style="width:600px; height:450px; padding-top:7px">'
    divhtml += '<form name="mail">'
    divhtml += '<div style="white-space:nowrap"><label for="topic">'+topic+': </label><input type="text" name="topic" id="topic" class="td_editable" style="width:95%" /></div>'
    divhtml += '<div style="padding-top:5px"><textarea id="txtEditHtmlArea" name="text" style="width:100%; height:360px;"/></div>'
    divhtml += '</form></div></div>'
    $("body").append(divhtml);
        ...
        buttons:{
            "Отправить": function(){
                if(document.getElementsByTagName("iframe")[0]){
                    $("#txtEditHtmlArea").val(document.getElementsByTagName("iframe")[0].contentDocument.getElementsByTagName("body")[0].innerHTML)
                }

                if($('input[name="topic"]').val() == "" && $('[name="text"]').val() == ""){
                    var confir = confirm(blankmessage)
                    if(confir){
                        sendMassage();
                        $(this).dialog('close');
                    }
    ...
    CKEDITOR.replace('txtEditHtmlArea',{width:'630px' , height:'270px'});
}

function sendMassage(){
   $("body").notify({"text":"Пожалуйста ждите... Идет отправка сообщения", "onopen":function(){} }) ;
    $("body").css("cursor", "wait");
    enableblockform()
    var data = $("form[name='mail']").serialize();
    var projectid = $("input[name='projectid']").val();
    $.ajax({
        type: "post",
        url: "Provider?type=page&id=mailtomanager&projectid=" + projectid + "&" + data,
        datatype: "xml",
        success: function(data){
            disableblockform();
            $("body").notify({hideAnimation: 'slideUp'});
            $("body").css("cursor", "default");
            isSent = $(data).find("mailresult").attr("sent")
            if(isSent == "true"){
                alert(sentMessage);
            }else{
                alert(notSentMessage)
            }
        },
        error: function(){
            $("body").notify({hideAnimation: 'slideUp'});
