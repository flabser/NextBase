...
function showCommentText(text){
	...
	divhtml+="<div style='text-align:center; white-space: pre-wrap;  white-space: -moz-pre-wrap;  white-space: -pre-wrap; white-space: -o-pre-wrap; word-wrap: break-word;'>"+
		"<font style='font-size:13px; width:100px'>"+text+"</font>"+"</div>";
	divhtml += "</div>";
	$("body").append(divhtml);
...
function controlOff(pos){
	...
	$("#controlres"+pos).val( execid +"`"+responsible+"`" +timeOff+":00" +"`"+idCorrControlOff);
}
...
function dialogConfirmComment (text,action){
	...
	divhtml ="<div><div id='dialog-message' title='"+warning+"'>";
...
function eds_attach_dialog(){
	...
	if ( $.cookie("lang")=="RUS" || !$.cookie("lang"))
	divhtml ="<div id='dialog-message-attach' title='Выбрать сертификат'>";
	else if ($.cookie("lang")=="KAZ")
		divhtml ="<div id='dialog-message-attach' title='Сертификатты таңдау'>";
	else if ( $.cookie("lang")=="ENG")
		divhtml ="<div id='dialog-message-attach' title='Select certificate'>";
	divhtml +="<span style='height:250px; width:100%;'><br/>"+
		...
		"Пароль: <input type='password' id='eds_pass_temp' size='15' value=''/>"+
		"<input type='hidden' name='formsesid' value='"+$("input[name=formsesid]").val()+"'/><br/>"+
		...
		"<input id='p12Path' type='text' readonly='readonly' style='width: 217px;' autocomplete='off' name='p12Path'/>"+
		"<input id='selectP12File' type='button' onclick='selectP12File();' style='margin-left:10px' value='Выбрать хранилище закрытого ключа' name='selectP12File'/>"+
		"<br/><style>.ui-progressbar .ui-progressbar-value { background-image: url(/SharedResources/jquery/css/base/images/pbar-ani.gif); }</style>"+
	...
	$("#dialog-message-attach").dialog({
		...
		buttons:{
			"Ок": function() {
				if($("[name=fname_eds]").val().length != 0){
...
function uploadComplete(tableID, doc) {
	...
	if (st = 'ok'){
		...
		if(fileName.indexOf(".") != -1){
			detectExtAttach(fileName); //определение расширения файла
			...
			"</td><td></td</tr>");
			if($("input[name=type]").val() != 'save_userprofile' && tableID != "upltable_eds"){
				ConfirmCommentToAttach(addcommentforattachment,csf)
			}else{
				$("#progressbar").progressbar("destroy");
				$("#progressstate").css("display","none");
				$("#commentaddimg"+csf).remove();
				fullpath=$(doc).find("message[id=3]").text();
				$("#fullpath").remove();
				$("body").append("<input type='hidden' id='fullpath' value='"+fullpath+"'/>")
			}
