...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="filename" select="document/fields/pdocrtfcontent"/>
		<html>
			<head>
				<title>
					<xsl:value-of select="document/fields/title"/> - Workflow документооборот
				</title>
					...
					$(document).ready(function(){
						<xsl:if test="$status  != 'new'">
							if($("#SignApplet").length !=0 ){
								...
								state = app.checkSign('<xsl:value-of select="document/fields/sign"/>','<xsl:value-of select="document/fields/taskauthorpk"/>', '<xsl:value-of select="document/fields/srctext"/>')
								switch(state){
									case 0:
										$("#eds_status").text('<xsl:value-of select="document/captions/documentnotsigned/@caption"/>')
										break;
									case 1:
										$("#eds_status").text('<xsl:value-of select="document/captions/signistrue/@caption"/>')
										$("#edsimg").attr("src","/SharedResources/img/iconset/key.png")
										$("#signtext, #signtexttr, #edsimg").css("visibility","visible")
										break;
									case 2:
										$("#eds_status").text('<xsl:value-of select="document/captions/signisfalse/@caption"/>')
										break;
									case 3:
										$("#eds_status").text('<xsl:value-of select="document/captions/invalidkey/@caption"/>')
										break;
									case 4:
										$("#eds_status").text('<xsl:value-of select="document/captions/algorithmnotfound/@caption"/>')
										break;
									case 5:
										$("#eds_status").text('<xsl:value-of select="document/captions/fillmechanismnotfound/@caption"/>')
										break;
									case 6:
										$("#eds_status").text('<xsl:value-of select="document/captions/invalidcharkey/@captions"/>')
										break;
									case 7:
										$("#eds_status").text('<xsl:value-of select="document/captions/invalidparalgo/@caption"/>')
										break;
									case 8:
										$("#eds_status").text('<xsl:value-of select="document/captions/totalexceptionkey/@caption"/>')
										break;
									case 10:
										$("#eds_status").text('<xsl:value-of select="document/captions/filecertnotfound/@caption"/>')
										break;
									case 11:
										$("#eds_status").text('<xsl:value-of select="document/captions/onefilenotfound4sign/@caption"/>')
										break;
									default:
										break;
								}
					...
					<script>
						$(function() {
							$('#ctrldate').datepicker({
								...
								regional:['ru'],
								showAnim: '',
								buttonText: "Орындау мерзімін өзгерту",
								...
								$("[name=dbd]").val(diff)
								if (diff >= 0){
									diff = "(Күндер қалды : " + diff+")"
									$(".diffdate").css("color","#000");
								}else{
									diff = "(Мерзiмiнен өткен күндер : " + diff.toString().split("-")[1] +")";
									$(".diffdate").css("color","red");
								}
								$(".diffdate").text(diff)
							}
					...
					<xsl:if test="/request/@lang = 'ENG'">
						<script>
							$(function() {
								...
								$("[name=dbd]").val(diff)
								if (diff >= 0){
									diff = "(Remained days : " + diff+")"
									$(".diffdate").css("color","#000");
								}else{
									diff = "(Delayed days : " + diff.toString().split("-")[1] +")";
									$(".diffdate").css("color","red");
								}
								$(".diffdate").text(diff)
							}
			...
			<body>
				<div id="docwrapper">
                    ...
                    <xsl:if test="document/@canbesign = '1'">
                    <applet id="SignApplet" code="kz.pchelka.digitalsign.Sign" width="1" height="1"
                            archive="digitalSignApplet.jar, log4j-1.2.15.jar, saxon9he.jar, servlet-api.jar, commons-codec-1.6.jar, iolajcp2.jar, xmlsec-1.4.4.jar, commons-logging-1.1.1.jar" codebase=".">
                    </applet>
                </xsl:if>
											...
											<td>
												<table>
													<tr>
														...
														<td>
															<input type="text" maxlength="10" class="td_noteditable" style="width:80px;" value="{substring(document/fields/control/primaryctrldate,1,10)}"/>
...
<!-- 												<input id="executor" style="width:600px"></input> -->
												...
												<xsl:for-each select="document/fields/execblock/executors/entry">
													<input type="hidden" name="executor" id="controlres{position()}" value="{shortname/@attrval}`{responsible}`{resetdate}`{resetauthorid}"/>
												</xsl:for-each>
									...
									<tr>
										...
										<td>
											<table id="intexectable" class="table-border-gray">
												...
												<xsl:for-each select="document/fields/execblock/executors/entry">
													<tr>
														<xsl:attribute name="id">exectr<xsl:value-of select="position()"/></xsl:attribute>
														<td style="width:3%;text-align:center">
															<xsl:value-of select="position()"/>
														</td>
														<td>
															<xsl:value-of select="shortname"/>
															<input type="hidden" id="idContrExec{position()}" value="{shortname/@attrval}"/>
															<input type="hidden" id="executorid" value="{shortname/@attrval}"/>
															<input type="hidden" id="controlOffDate" value="{resetdate}"/>
															<input type="hidden" id="responsible" value="{responsible}"/>
															<input type="hidden" id="resetauthor" value="{resetauthorid}"/>
															<input type="hidden" id="execpercent" value="{execpercent}"/>
														</td>
														<td id="controlOffDate{position()}">
															<xsl:value-of select="resetdate"/>
														</td>
														<td id="idCorrControlOff{position()}">
									...
									<xsl:if test="$status !='new'">
										<div display="block" style="display:block; width:90%" id="execution">
											...
											<font style="font-style:arial; font-size:13px">
												<b><xsl:value-of select="document/fields/title"/></b>
											</font>
