									...
									<form action="Provider" name="frm" method="post" id="frm" enctype="application/x-www-form-urlencoded">
										<div class="ui-tabs-panel" id="tabs-1">
											...
											<table width="100%" border="0">
												...
												<!-- поле "Категория"
												<tr>
													<td class="fc"><xsl:value-of select="document/captions/cat/@caption"/> :</td>
													<td>
														<xsl:variable name="cat" select="document/fields/cat/@attrval"/>
														<select name="cat" class="select_editable" style="width:611px;">
															<xsl:if test="document/@editmode != 'edit'">
																<xsl:attribute name="disabled"/>
																<xsl:attribute name="class">select_noteditable</xsl:attribute>
																<option value="">
																	<xsl:attribute name="selected">selected</xsl:attribute>
																	<xsl:value-of select="document/fields/cat"/>
																</option>
															</xsl:if>
