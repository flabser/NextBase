...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<html>
			<head>
				<title>
					<xsl:value-of select="document/captions/name/@caption"/> - Workflow документооборот
				</title>
					...
					<div class="button_panel">
						<span style="float:left">
							...
							<button title ="{document/captions/saveclose/@hint}" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="btnsavedoc">
								<xsl:attribute name="onclick">javascript:SaveFormJquery('frm','frm',"<xsl:value-of select="history/entry[@type = 'page'][last()]"/>")</xsl:attribute>
								<span>
