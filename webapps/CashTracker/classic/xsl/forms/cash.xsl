					...
					<div class="fieldset-container">
						...
						<div class="control-group">
							<div class="control-label">
								...
								<xsl:if test="$editmode = 'edit'">
									<button type="button" class="btn-picklist" title="{document/captions/user/@caption}" onclick="nbApp.dialogChoiceBossAndDemp(this, 'user', false)" />
								</xsl:if>
							</div>
							<div class="controls">
								<xsl:call-template name="field">
									<xsl:with-param name="name" select="'user'" />
									<xsl:with-param name="node" select="document/fields/user" />
								</xsl:call-template>
							</div>
						</div>
						<div class="control-group">
							<div class="control-label">
								<xsl:value-of select="document/captions/amountcontrol/@caption" />
							</div>
							<div class="controls">
								<input type="text" name="amountcontrol" value="{document/fields/amountcontrol}" class="span2" />
							</div>
						</div>
						<div class="control-group">
							<div class="control-label">
