...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request/query/entry">
		<div style="cursor:pointer; text-align:left" name="itemStruct">
			...
			<input type="checkbox" name="chbox" class="{@doctype}~{@docid}" id="{@docid}" value="{@viewtext}"/>
			<font class="font" title="{userid}" style="font-family:verdana;  font-size:14px; margin-left:2px">
				<xsl:if test="userid =''">
