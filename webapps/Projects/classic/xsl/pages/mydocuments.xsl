...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template match="//query/entry">
		...
		<tr title="{@viewtext}" class="{@docid}" id="{@docid}{@doctype}">
			<xsl:choose>
				<xsl:when test ="//request/@id='completetasks'">
                ...
                <a href="{@url}" title="{viewcontent/viewtext}" class="doclink">
                    ...
                    <font id="font{@docid}{@doctype}">
                        <script>
                            ...
                            if(control == '2'){
                            $("#"+ids+" > .control").attr("src","/SharedResources/img/iconset/tick_gray.png").attr("title","<xsl:value-of select='//captions/readytocontroloff/@caption' />");
                            }
			...
			<td style="border:1px solid #ccc; border-left:none; width:30px; text-align:center">
                ...
                <xsl:if test="@topicid != '0'">
                    <a title="{//captions/viewdiscussion/@caption}">
                        ...
                        <img class="favicon" style="cursor:pointer; width:18px; height:18px" src="/SharedResources/img/classic/icons/comment.png"/>
                    </a>
                </xsl:if>
            </td>
		</tr>
