...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<html>
			...
			<body>
				...
				<div id="wrapper">	
					...
					<!-- <xsl:value-of select="page/view_content/query/@flashdocid"/> -->
						...
						<div id="viewcontent">
							<div id="viewcontent-header" style="height:68px; position:relative;">
								...
								<div id="QFilter" style="width:auto; background: #eeeeee; border:1px solid #ccc; border-bottom:none; height:27px; margin-top:5px; margin-left:2px; display:block; position:absolute; top:81px; right:0px; left:12px">
									...
									<font style="font-size:11px; line-height:25px; margin-left:5px"><xsl:value-of select="$captions/tester/@caption"/> :</font>
									...
									<font style="font-size:11px; line-height:25px; margin-left:5px"><xsl:value-of select="$captions/programmer/@caption"/> :</font>
									  	...
									  	<font id="catlistbutton" class="listbutton">
											...
											<font style="font-size:11px; font-weight:bold">
												<xsl:if test="$query/columns/viewtext2/filter/@mode = 'ON'">
													<xsl:value-of select="$query/columns/viewtext2/filter/@keyword"/>
												</xsl:if>
												<xsl:if test="$query/columns/viewtext2/filter/@mode = 'OFF'"><xsl:value-of select="$captions/all/@caption"/></xsl:if>
											</font>
										</font>
									</span>
									<div id="catlist" class="glosslisttable" style="position:absolute; top:90px; z-index:999; border:1px solid #79B7E7;  width:300px; background:#fff; display:none; max-height:400px; overflow:auto">
										<table style="width:100%">
											...
											<tr style="font-family:Verdana,Arial,Helvetica,sans-serif; font-size:13px; min-width:120px;">
												<td class="categorylist_td" style=" cursor:pointer; border-top: 1px solid #dddddd;">
													<xsl:attribute name="onclick">javascript:resetCurrentFilter('viewtext2')</xsl:attribute>
													<font style=" width:100%; font-color:#555">
														<xsl:value-of select="$captions/all/@caption"/>
													</font>
												</td>
			        				...
			        				<div style="display:inline-block; width:20px"/>
			        				<font style="font-size:11px; line-height:25px;"><xsl:value-of select="$captions/manager/@caption"/> :</font>
									<span class="statuslistbuttonwrapper">
									  	<font id="statuslistbutton" class="listbutton">
											...
											<font style="font-size:11px; font-weight:bold">
												<xsl:if test="$query/columns/viewtext3/filter/@mode = 'ON'">
													<xsl:value-of select="$query/columns/viewtext3/filter/@keyword"/>
												</xsl:if>
												<xsl:if test="$query/columns/viewtext3/filter/@mode = 'OFF'"><xsl:value-of select="$captions/all/@caption"/></xsl:if>
											</font>
										</font>
									</span>
									<div id="statuslist" class="glosslisttable" style="position:absolute; top:90px; z-index:999; border:1px solid #79B7E7;  width:300px; background:#fff; display:none; max-height:400px; overflow:auto">
										<table style="width:100%">
											...
											<tr style="font-family:Verdana,Arial,Helvetica,sans-serif; font-size:13px; min-width:120px;">
												<td class="categorylist_td" style=" cursor:pointer; border-top: 1px solid #dddddd;">
													<xsl:attribute name="onclick">javascript:resetCurrentFilter('viewtext3')</xsl:attribute>
													<font style=" width:100%; font-color:#555">
														<xsl:value-of select="$captions/all/@caption"/>
													</font>
												</td>
			        				...
			        				<xsl:if test="//filter/@mode = 'ON'">
					        			<img src="/SharedResources/img/iconset/054-delete.png" style="margin-left:10px; cursor:pointer; vertical-align:-9px; border:1px solid #cdcdcd; padding:3px 4px; border-radius:3px">
	                                        <xsl:attribute name="title" select="//captions/removefilter/@caption"/>
	...
	<xsl:template match="//query/entry">
		...
		<tr title="{@viewtext}" class="{@docid}" id="{@docid}{@doctype}">
			...
			<td  style="border-bottom:1px solid #ccc;border-top:1px solid #ccc; min-width:160px;">
				<xsl:if test="@hasresponse='1'">
					<img style="vertical-align:-4px; margin-left:2px; margin-right:5px; border:0px; cursor:pointer" src="/SharedResources/img/classic/1/plus1.png" docid="{@docid}" doctype="{@doctype}">
						<xsl:attribute name='onclick'>javascript:openParentDocViewProject(this)</xsl:attribute>
						<xsl:if test=".[responses]">
							<xsl:attribute name='src'>/SharedResources/img/classic/1/minus1.png</xsl:attribute>
							<xsl:attribute name='onclick'>javascript:closeResponsesProject(this)</xsl:attribute>
						</xsl:if>
					</img>
				</xsl:if>
			...
			<td style="border:1px solid #ccc; width:200px;">
				<a  href="{@url}" class="doclink" style="padding-left:25px;">
				<xsl:if test="@isread = 0">
					<xsl:attribute name="style">font-weight:bold; padding-left:20px;</xsl:attribute>
				</xsl:if> 
				<xsl:value-of select="viewcontent/viewdate"/></a>
			</td>
	...
	<xsl:template match="entry" mode="line">
		<xsl:if test="@form = 'milestone' or @form = 'projectDoc'">
			<div class="Node" style="overflow:hidden; height:22px" id="{@docid}{@doctype}">
				<xsl:if test="@form = 'milestone'">
					<xsl:attribute name="style">overflow:hidden; height:18px;background-color:#F8F8FF;</xsl:attribute>
				</xsl:if>
	...
	<xsl:template match="entry" mode="item">						
			...
			<a href="{@url}" title="{viewcontent/viewtext}" class="doclink" style="font-style:Verdana,​Arial,​Helvetica,​sans-serif; width:100%; font-size:97%; margin-left:2px">
				...
				<xsl:if test="@hasattach != 0">
					<img id="atach" src="/SharedResources/img/classic/icons/attach.png" height="14" border="0" style="vertical-align:top; margin-left:2px; margin-right:3px">
