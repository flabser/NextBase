...
function addDocToFav(el,docid,doctype){
	$.ajax({
		...
		datatype:"XML",
		url: "Provider",
		data: "type=service&operation=add_to_favourites&id=add_to_favourites&key="+docid+"&doctype="+doctype+"&id=demand",
		cache:false,
...
function removeDocFromFav(el,docid,doctype){
	$.ajax({
		...
		datatype:"XML",
		url: "Provider",
		data: "type=service&operation=remove_from_favourites&id=remove_from_favourites&key="+docid+"&doctype="+doctype+"&id=demand",
		cache:false,
		success: function (msg){
...
function searchCompleteDemands(){
	$("#tablecontent").empty();
	loadingOutline()
	$.ajax({
		type: "GET",
		datatype:"XML",
		url: "Provider",
		data: "type=page&id=statistics_complete_demand_calc&emp="+$("select[name=emp]").val()+"&startdate="+$("input[name=startdate]").val()+"&enddate="+$("input[name=enddate]").val(),
		cache:false,
		...
		error: function(data,status,xhr) {
			endLoadingOutline()
			infoDialog("Ошибка поиска исполненных заявок");
		}
	})
}
...
function openParentDocViewProject(element) {
	loadingOutline()
	$("body").css("cursor","wait");
	var elem = null,
		tr = $(element).closest("tr"),
	...
	$("<tr id='loadingparentdoc"+ docid +"' style='background: #fff'><td colspan='"+ $(element).children("td").length +"' style='text-align:center'><img src='classic/img/image_311968.gif'/></td></tr>").insertAfter(tr);
	$.ajax({
		url: 'Provider?type=view&id=docthread_project&parentdocid='+docid+'&parentdoctype='+doctype+'&command=expand`'+docid+'`'+doctype,
		datatype:'html',
		  success: function(data) {
			 elem = $(data);
			 $(element).closest("td").prevAll("td").each(function(){
	...
	$(element).attr("src","/SharedResources/img/classic/1/minus1.png").attr("onclick","javascript:closeResponsesProject(this)");
}

function closeResponsesProject(element){
	var docid = $(element).attr("docid"),
		doctype = $(element).attr("doctype");
	$.get('Provider?type=view&id=docthread_project&command=collaps`'+docid+'`'+doctype, {})
	var tr = $(element).closest("tr").next("tr");
	$(tr).remove();
	$(element).attr("onclick","javascript:openParentDocViewProject(this)").attr("src","/SharedResources/img/classic/1/plus1.png");
}
...
function checkAllOrgMembers(allChbox){
	allChbox.checked ? $("input[class="+allChbox.id+"]").attr("checked","true") : $("input[class="+allChbox.id+"]").removeAttr("checked");
}
...
function chooseFilter(keyword,column){
	url= 'Provider?type=service&operation=tune_session&element=page&id='+outline.viewid+'&param=filter_mode~'+'on'+'&param=filtered_column~'+column+'&param=key_word~'+encodeURIComponent(keyword);
	$.ajax({
		url: url,
		dataType:'xml',
		async:'true',
...
function resetCurrentFilter(column){
	url= 'Provider?type=service&operation=tune_session&element=page&id='+outline.viewid+'&param=filtered_column~'+column+'&param=filter_mode~on';
	$.ajax({
		url: url,
		dataType:'xml',
		cache:false,
...
function openCategoryList(el, listid){
	$(".glosslisttable").css("display", "none");
	$(el).offset(function(i,val){
		$("#"+listid).css("position", "absolute");
		if(IE='\v'=='v'){
			$("#"+listid).css("top", val.top - 150);
		}else{
...
function updateView(viewid, page, command,  sortField, sortOrder){
	loadingOutline()
	...
	url= 'Provider?type=page&id=' + outline.viewid + '&page=' + outline.curPage + commandPart+ sortPart+"&keyword="+category;
	if ($.cookie("lang")=="RUS"){
...
function subentry(id) {
	$("subentry" + id).style.display == "none" ? $('subentry' + id).style.display = "block" : $('subentry' + id).style.display = "block";
}
	...
	if(type== 'statistics_staff_workload'){
		...
		$.ajax({
			...
			success: function (msg){
				var items = new Array(),
					item = new Object(),
					entries = $(msg).find("statistics").find("entry");
		...
		$.ajax({
			...
			success: function (msg){
				...
				if(entries.length !=0){
					...
					chart1.getToolTips().setEnabled(false);
				    var chartDiv = document.getElementById('barStat');
				    if(entries.length/2 > 6){
				    	var new_height = 30*((entries.length -12)/2)+450;
				    	chartDiv.style.height = new_height+'px';
				    }
			        chart1.create(chartDiv);
				 }else{
					$("#barStat").append("<font style='font-size:11px;color:#555555'>No Data Available</font>")
