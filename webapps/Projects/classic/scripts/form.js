...
var demandrevoked ="Заявка отменена";
var demandchanged ="Содержание заявки изменено";
var showhistoryctrldate= 'Отобразить историю изменения';
var hidehistoryctrldate= 'Скрыть историю изменения';
...
function resetcontrol(docid, page){
	...
	divhtml += notifyofnewimplement+"</div></div>";
	$("body").append(divhtml);
	if ($.cookie("lang")=='ENG'){
		...
		$.ajax({
			...
			success:function (xml){
				...
				if(resp){
					...
					$("#topic_"+parentdocid+" #msgWrapper").append('<div class="msgEntry" id="msgEntry'+ count  +'"/>')
				}
				$("#topic_"+parentdocid+" #msgEntry"+count).append('<div class="headermsg" id="headermsg'+ count +'"/>');
				$("#topic_"+parentdocid+" #headermsg"+count).append('<div class="authormsg">'+$("#username").val()+'</div>');
				$("#topic_"+parentdocid+" #headermsg"+count).append('<div class="msgdate">'+sent+':'+$("#postdate").val()+'</div>');
				$("#topic_"+parentdocid+" #msgEntry"+count).append('<div class="bodymsg" id="bodymsg'+ count +'">'+$("#commentvalue").val()+'</div>');
				$("#topic_"+parentdocid+" #msgEntry"+count).append('<div class="buttonpanemsg" id="buttonpanemsg'+ count +'"><button type="button" onclick="javascript:addCommentToForum(this,'+ $(xml).find("message[id=2]").text()+',905,true)" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only commenttocomment " style="float:right; margin-top:3px"><font style="font-size:12px; vertical-align:top" >'+makecomment+'</font></button></div>')
				$("#topic_"+parentdocid+" #buttonpanemsg"+ count+" button").button();
				if(resp){
					$("#commentform").slideUp("fast", function() {
						i = count+1;
					...
					$("#commentform").slideUp("fast", function() {
						i = count+1;
						$("#topic_"+parentdocid+" #CountMsgTheme").html(comment_on_discussion +":" + i);
						$("#commentform").remove();
						$("#docwrapper").animate({ scrollTop: $('#topic_'+parentdocid+' #msgEntry'+count).position().top}, "slow",function() {
							//infoDialog("Комментарий успешно добавлен")
						}); 
					}); 
				}
...
function toogleHistoryProlongation(){
	if($("#historyprolongation").css("display") == 'none'){
		...
		$("#historyprolongationCaption").text(showhistoryctrldate).attr("title",showhistoryctrldate)
	}
}
				...
				success: function(data) {
					...
					$(data).find("techsupervision").find("entry").each(function(index, element){
						...
						$("#projectextratable").append("<tr class='tehnadzortr'>"+
							"<td width='30%' class='fc'>"+ technadzor +"</td>" +
...
function AddFields(formname, parent_doctype, parent_docid, newdivid){	
	remove_text = "";
	lang == 'KAZ' ? remove_text = "Алып тастау" : remove_text = "Убрать";
	if(formname == 'animals' || formname == 'farmSownArea'){
		...
		$.ajax({
			...
			success: function(data){
				$("#"+newdivid).append("<form action='Provider' enctype='application/x-www-form-urlencoded' method ='post' name='frm" 
						+ fieldCounter+"' id='frm" +fieldCounter+"'/>");
				$("#frm" + fieldCounter).append(data);	
				$("#"+newdivid).append('<div id ="extradiv' +fieldCounter +'" style="margin: -98px 0 0 1050px"><button title="'+remove_text+'" onclick="RemoveField('+fieldCounter+')" id="removefield' + fieldCounter +'"> <span><img src="/SharedResources/img/classic/icons/delete.png" height="15px" /></span></button><div style="margin-top: 90px" /></div>');
				$("#"+newdivid).append('');
				formsList += fieldCounter + "*";
...
function SaveMultipleForms(typeForm, formName, redirecturl, docType){
	...
	for(var i=0; i<arr.length; i++){				
		...
		$.ajax({
			...
			success:function (xml){ 
				redir = $(xml).find('history').find("entry[type=view]:last").text() || redirecturl || redirectAfterSave;
				$(xml).find('response').each(function(){
					var st=$(this).attr('status');
					...
					if (st == "ok"){
						...
						if (msgtext.length!=0){
							lang == 'KAZ' ? msgtext = "Құжат сақталды" : msgtext = "Документ сохранен";
						}
						$("#dialog-message").html("<br/><div style='text-align:center'><font style='font-size:13px; font-family:Verdana,Arial,Helvetica,sans-serif; padding-left:10px'>"+msgtext+"</font></div>");
	...
	if(filePath != ""){
		$.ajax({
			...
			success: function (msg){			  
				...
				result == "wrongFile" ? infoDialog(wrongFileSelected) : infoDialog(successImport + "<br/>"+ region_text + ": "+ sv);
			},
			error: function(data,status,xhr) {
...
function changeCtrlDate(priority, complication, startdate){
	$.ajax({
		type: "GET",
...
// Ограничение даты "Приступить к работе с"
function startDateLimit(date){
    $( "#startDate" ).datepicker( "option", "maxDate", new Date(date));
}
...
function changeBriefcontentDemand(docid,doctype){
	...
	if($.cookie("lang")=="ENG"){
		title ='Change content to demand';
	}else if ($.cookie("lang")=="KAZ"){
		title ='Мазмұнын өзгерту'
	}
	divhtml = "<div id='dialog-revoke' style='display:none' title='"+title+"'>";
	divhtml += "<div style='height:400px; width:98%; font-size:14px;'>";
	divhtml += "Содержание:<div style='height:10px'/>"; 
	divhtml += '<form name="formbriefcontent"><div>';
	divhtml += '<textarea id="changedbriefcontent" name="briefcontent" style="width:670px; height:360px;">'+ $("[name=briefcontent]").val()+'</textarea>';
	divhtml += '</div></form>';
	
	$("body").append(divhtml);
	if($(".android_text").length == 0){
	 	CKEDITOR.replace('changedbriefcontent',{width:'670px', height:'270px'}); 
	}
	if ($.cookie("lang") == "RUS" || !$.cookie("lang")){
		$("#dialog-revoke").dialog({
			...
			buttons: {			
				'Ок': function() {
					if($(".cke_wysiwyg_frame")){
						$("#changedbriefcontent").val($(".cke_wysiwyg_frame").contents().find("body").html())
					}
					changeBriefcontentDemandOK(docid, doctype, $("form[name=formbriefcontent]").serialize());
					$(this).dialog('close');
					return false;
				},'Отмена': function() {
					$(this).dialog('close');
...
function revokeDemand(docid, doctype){	 
	...
	if($.cookie("lang")=="ENG"){
		title = "Cancel demand";
		reason_caption = "Reason"; 
	}else if ($.cookie("lang")=="KAZ"){
		title = "Тапсырысты болдырмау";
		reason_caption = "Себебі"; 
	}
	
	divhtml = "<div id='dialog-revoke' title='"+title+"'>";
	divhtml += "<div style='height:200px; width:98%; font-size:14px;'>";
	divhtml += reason_caption+":<div style='height:10px'/>"; 
...
function extendDemand(docid, doctype){	
	...
	$.ajax({
		...
		success: function (msg){
			...
			select_values=[1,2,3,4,5,10,15,30];
			for (var i = 0; i < select_values.length; i++) {
				divhtml +="<option value='"+select_values[i]+"'>"+select_values[i]+"</option>"
			}
			divhtml +="</select></div>";
			$("body").append(divhtml);
...
function extendDemandOK(docid, doctype, reasonID){
	...
	$.ajax({
		...
		datatype:"xml",
		url: "Provider",
		data: "type=page&id=extend-demand-days&key="+docid+"&doctype="+doctype+"&extend_demand_days="+n+"&extend_demand_reason_id="+reasonID,
		success: function (msg){
			if($(msg).find("response").attr("status") == "ok"){
