...
function changeEmpListText(el, vt){
    if(el.checked){
        txt = $(".selectedNames").html();
        if($(el).attr('id') == 'allchbox'){
            $(".selectedNames").html(vt)
        }else{
            $(".selectedNames").html(txt + (txt != "" ? ", " : "") + vt);
            $(".selectedNames").attr("title", $(".selectedNames").html())
        }
    }else{
        if($(el).attr('id') == 'allchbox'){
            $(".selectedNames").html("");
            ...
            txt = $(".selectedNames").html();
            if(txt.indexOf(", " + vt) != -1)
                $(".selectedNames").html(txt.replace(", " + vt,''));
            else
                $(".selectedNames").html(txt.replace(vt,''));
            $(".selectedNames").attr("title", $(".selectedNames").html())
        }
