...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="header-page">	
		<div id="header-view">
			<span >
				<font style="font-size:4.5em; color:#202020; margin-left:40px; font-family: Sakkal Majalla ,tahoma,sans-serif; line-height:100px">Whiteboard</font>
			</span>
			<span style="float:right; padding:30px 5px 5px 0px">
				<a id="currentuser" target="_parent" href="Provider?type=edit&amp;element=userprofile&amp;id=userprofile"  style="color: #444444 ; font:16px Arial; font-weight:bold; vertical-align:10px">
					<xsl:attribute name="title"><xsl:value-of select="page/captions/view_userprofile/@caption"/></xsl:attribute>
					<xsl:value-of select="@username"/>
				</a>
				<a id="currentuser" target="_parent" href="Provider?type=edit&amp;element=userprofile&amp;id=userprofile" style="text-decoration:none; margin-left:1px">
					<xsl:attribute name="title"><xsl:value-of select="page/captions/view_userprofile/@caption"/></xsl:attribute>
					<img src="/SharedResources/img/font-awesome/png/cog2.png" style="height:36px"/>
				</a>
				<a target="_parent" href="Logout" title="{page/captions/logout/@caption}" id="logout"  style="text-decoration:none; margin-left:1px">
					<xsl:attribute name ="title"><xsl:value-of select ="page/captions/logout/@caption"/></xsl:attribute>
					<img src="/SharedResources/img/font-awesome/png/sign3.png" style="height:36px"/>
				</a>
				<a target="_parent" id="helpbtn" title="{page/captions/helpcaption/@caption}" href="/Help/Provider?type=page&amp;id=article" style="text-decoration:none; margin-left:1px">
					<xsl:attribute name ="title"><xsl:value-of select ="page/captions/helpcaption/@caption"/></xsl:attribute>
					<img src="/SharedResources/img/font-awesome/png/question23.png" style="height:36px"/>
				</a>
			</span>				
		</div>
	</xsl:template>
    ...
    <xsl:template name="outline-section-state">
        <script>
            if($.cookie("WORKFLOW_<xsl:value-of select='@id'/>") != 'null'){
                ...
                if($.cookie("WORKFLOW_<xsl:value-of select='@id'/>") == "none"){
                ...
                }else if($.cookie("WORKFLOW_<xsl:value-of select='@id'/>") == "block"){
                    ...
                    <xsl:if test="name() = 'entry'">
                        $("img.<xsl:value-of select='@id'/>toogle_img").attr("src","/SharedResources/img/classic/1/plus.png")
                        $("#<xsl:value-of select='@id'/>").css("display", "none")
                    </xsl:if>
                }
            }
        </script>
