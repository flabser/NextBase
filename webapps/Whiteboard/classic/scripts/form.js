...
function checkPassword(storeType, fullPath){// проверка пароля
    var ret = false;
    try{
                ...
                if(filePath.length != 0){
                    if($("#eds_pass_temp").val().length != 0){
                    	...
                    	if(checkPassword(storeType, fileOrDevice)){
                        	...
                        	$.ajax({
                    			type: "POST",
                    			url: 'SignProvider?type=service&operation=check_cert&cert='+cert,
	...
	if (st = 'ok'){
		...
		if(fileName.indexOf(".") != -1){
			...
			$(table).append("<tr id='"+ csf +"'>" +
					'<td><input type="hidden" name="filename" value="'+fileName+'"/><input type="hidden" name="fileid" value="'+fileid+'"/></td>' +
					"<td>" +
