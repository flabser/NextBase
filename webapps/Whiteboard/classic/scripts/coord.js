...
function addCoord(){
	...
	"<div class='contentCoord'>" +
		...
		"<div class='buttonPane button_panel' style='width:100%;'>" +
			"<span style='float:right; margin-right:11px; margin-top:3px; margin-bottom:14px'><button class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onclick='coordOk()' style='margin-right:5px'><span class='ui-button-text'><font style='font-size:12px; vertical-align:top'>ОК</font></span></button>" +
			"<button class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onclick='hideDialog()'><span class='ui-button-text'><font style='font-size:12px; vertical-align:top'>"+cancelcaption+"</font></span></button>" +
			"</a></span>" +
		"</div>"+
	"</div>";
	$("body").append(divhtml);
...
function decision(yesno, key, action){
    ...
    if (yesno == "no"){
    	...
    	if ($.cookie("lang")=="RUS" || !$.cookie("lang"))
    		dialogConfirmComment ("Оставить комментарий ответа?",action)
    	else if ($.cookie("lang")=="KAZ")
        	dialogConfirmComment ("Жауаптың түсiнiктемесін қалдырасыз ма?",action)
        else if ($.cookie("lang")=="ENG")
        	dialogConfirmComment ("To leave the answer comment?",action)
    }
