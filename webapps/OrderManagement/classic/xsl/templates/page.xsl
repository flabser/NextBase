...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="outline-section-state">
		<script>
			if($.cookie("OrderManagment_<xsl:value-of select='@id'/>") != 'null'){
				$("#<xsl:value-of select='@id'/>").css("display",$.cookie("OrderManagment_<xsl:value-of select='@id'/>"))
				if($.cookie("OrderManagment_<xsl:value-of select='@id'/>") == "none"){
	...
	<xsl:template name="header-view">	
		<div style="position:absolute; top:0px; left:0px; width:100%; height:70px; border-bottom:1px solid #ADD8E6; background:url('classic/img/back.png'); ">
	     	<span style="float:left">
		     	<img src="/SharedResources/logos/OrderManagment_logo.png" style="margin:5px 0px 0px 10px; height:60px"/>
			</span>
	     	 
			<span style="float:left; margin:25px 0 0 0px">
				<font style="font-size:1.6em; color:black; padding-left: 10px">OrderManagment</font>
				<font style="font-size:1em; color:black; margin-left:10px;">
			...
			<span style="float:right; padding:5px 5px 5px 0px;text-align:right" >
				...
				<a href="Logout" id="logout" style="text-decoration:none; color:#555555; font:11px Tahoma; font-weight:bold; margin:0 5px 0 10px">
 				...
 				<a id="helpbtn" href="Provider?type=static&amp;id=help_summary" style="text-decoration:none; color:#555555; font:11px Tahoma; font-weight:bold; margin-left:15px">
 					<xsl:attribute name="title"><xsl:value-of select ="//captions/help/@caption"/></xsl:attribute>
