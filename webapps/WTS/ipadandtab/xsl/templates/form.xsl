...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="cssandjs">
		...
		<link type="text/css" rel="stylesheet" media="screen" href="ipadandtab/css/form.css"/>
		<link type="text/css" rel="stylesheet" media="screen" href="ipadandtab/css/actionbar.css"/>
		<link type="text/css" rel="stylesheet" media="screen" href="ipadandtab/css/dialogs.css"/>
		<link type="text/css" rel="stylesheet" media="screen" href="ipadandtab/css/main.css"/>
		...
		<script type="text/javascript" src="ipadandtab/scripts/form.js?ver=1"/>
		<script type="text/javascript" src="ipadandtab/scripts/coord.js?ver=1"/>
		<script type="text/javascript" src="ipadandtab/scripts/dialogs.js?ver=1"/>
		<script type="text/javascript" src="ipadandtab/scripts/outline.js?ver=1"/>
	...
	<xsl:template name="documentheader">
		<div style="position:absolute; top:0px; left:0px; width:100%; background:url(/WTS/classic/img/yellow_background.jpg); height:70px; border-bottom:1px solid #fcdd76; z-index:2">
			<span style="float:left">
				...
				<font id="apptitle" style="font-size:1.2em; vertical-align:25px; color:#555555"><xsl:value-of select="document/captions/appcaption/@caption"/></font>
			</span>
				...
				<xsl:for-each select="document/fields/rtfcontent/entry">
					<tr>
						...
						<xsl:attribute name='id' select="$id"/>
							<td width="12%"></td>
							<td colspan="2">
								<div class="test" style="width:99%; overflow:hidden; display:inline-block">
								<xsl:choose>
									<xsl:when test="$extension = 'jpg' or $extension = 'jpeg' or $extension = 'gif' or $extension = 'bmp' or $extension = 'png'">
										<img class="imgAtt" title="{$filename}" style="border:1px solid lightgray; max-width:700px; max-height:600px; margin-bottom:5px">
										...
										<xsl:if test="/request/document/@editmode = 'edit'">
											<xsl:if test="comment =''">
												<a href=''  style="position:absolute; top:95px; margin-left:15px">
													<xsl:attribute name='href'>javascript:addCommentToAttach('<xsl:value-of select="$id"/>')</xsl:attribute>
													<img id="commentaddimg{$id}" src="/SharedResources/img/classic/icons/comment_add.png" style="width:16px; height:16px" title="Добавить комментарий"/>
												</a>
											</xsl:if>
											<a href='' style="position:absolute; top:355px; margin-left:25px">
									...
									<xsl:otherwise>
										...
										<xsl:if test="/request/document/@editmode = 'edit'">
											<xsl:if test="comment =''">
												<a href='' style="vertical-align:2px;">
													<xsl:attribute name='href'>javascript:addCommentToAttach('<xsl:value-of select="$id"/>')</xsl:attribute>
													<img id="commentaddimg{$id}" src="/SharedResources/img/classic/icons/comment_add.png" style="width:16px; height:16px" title="Добавить комментарий"/>
												</a>
											</xsl:if>
											<a href='' style="vertical-align:2px; margin-left:8px">
												<xsl:attribute name='href'>javascript:deleterow('<xsl:value-of select="$formsesid"/>','<xsl:value-of select='$filename'/>','<xsl:value-of select="$id" />')</xsl:attribute>
												<img src="/SharedResources/img/iconset/cross.png" style="width:13px; height:13px"/>
											</a>
										</xsl:if>
