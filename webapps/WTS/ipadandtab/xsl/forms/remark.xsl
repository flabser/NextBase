	...
	<body>
		...
		<div id="docwrapper" >
			<xsl:call-template name="documentheader"/>	
				<div class="formwrapper" style="overflow:visible">
					<div class="formtitle">
						<!-- заголовок -->					
						...
						<span style="vertical-align:12px; float:left">
							...
							<xsl:if test="document/actions/action [.='COMPOSE_EXECUTION']/@enable = 'true'">
								<button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="btnexecution" style="margin-left:5px" title="{document/actions/action [.='COMPOSE_EXECUTION']/@hint}">
									<xsl:attribute name="onclick">javascript:window.location.href="Provider?type=edit&amp;id=execution&amp;key=&amp;parentdocid=<xsl:value-of select="document/@docid"/>&amp;parentdoctype=<xsl:value-of select="document/@doctype"/>&amp;page=null"</xsl:attribute>
									<span>
									...
									<xsl:if test="not(document/fields/category/@mode = 'hide')">
										<tr>
											...
											<td style="padding-top:5px">
												...
												<select size="1" id="category" name="category" style="width:610px;" class="select_editable" onchange="javascript:addSubCatGloss(this)">
													<xsl:if test="$editmode != 'edit'">
														<xsl:attribute name="disabled">true</xsl:attribute>
														<xsl:attribute name="class">select_noteditable</xsl:attribute>
														<option value=" ">
