...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="query/entry">
		...
		<tr class="{@docid}" id="{@docid}{@doctype}" onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
			...
			<td style="text-align:center; border:1px solid #ccc; width:40px; padding:12px 5px" width="3%" >
								...
								<xsl:if test="query/@maxpage !=1">
									<xsl:if test="$curpage !=1">
										<button style="margin-left:4px" title="{columns/column[@id = 'FASTFILTERCAPTION']/@caption}" id="btnQFilter">
											<xsl:attribute name="onclick">javascript:window.location.href='Provider?type=search&amp;keyword=<xsl:value-of select='query/@keyword'/>&amp;page=1'</xsl:attribute>
											<font style="font-size:14px; vertical-align:top">&lt;&lt;</font>
										</button>
										<button style="margin-left:4px" title="{columns/column[@id = 'FASTFILTERCAPTION']/@caption}" id="btnQFilter">
											<xsl:attribute name="onclick">javascript:window.location.href='Provider?type=search&amp;keyword=<xsl:value-of select='query/@keyword'/>&amp;page=<xsl:value-of select='$prevpage'/>'</xsl:attribute>
											<font style="font-size:14px; vertical-align:top">&lt;</font>
										</button>
									</xsl:if>
									<select style="height:41px;width:60px; vertical-align:-13px; margin-left:4px; padding-left:15px">
										<xsl:attribute name="onChange">jjavascript:window.location.href="Provider?type=search&amp;keyword=<xsl:value-of select='query/@keyword'/>&amp;page="+this.value</xsl:attribute>
					 					<xsl:call-template name="combobox"/>
			 						</select>
			 						<xsl:if test="$curpage != $maxpage">
				 						<button style="margin-left:4px" title="{columns/column[@id = 'FASTFILTERCAPTION']/@caption}" id="btnQFilter">
											<xsl:attribute name="onclick">javascript:window.location.href='Provider?type=search&amp;keyword=<xsl:value-of select='query/@keyword'/>&amp;page=<xsl:value-of select='$nextpage'/>'</xsl:attribute>
											<font style="font-size:14px; vertical-align:top">&gt;</font>
										</button>
										<button style="margin-left:4px" title="{columns/column[@id = 'FASTFILTERCAPTION']/@caption}" id="btnQFilter">
											<xsl:attribute name="onclick">javascript:window.location.href='Provider?type=search&amp;keyword=<xsl:value-of select='query/@keyword'/>&amp;page=<xsl:value-of select='$maxpage'/>'</xsl:attribute>
											<font style="font-size:14px; vertical-align:top">&gt;&gt;</font>
										</button>
									</xsl:if>
								</xsl:if>
					...
					<div id="viewtablediv">
						<div id="tableheader">
							<table class="viewtable" id="viewtable" width="100%">
								<tr class="th">
									<td style="text-align:center; padding:12px 5px; width:40px" width="3%" class="thcell">
										<input type="checkbox" id="allchbox" onClick="checkAll(this);"  style="width:20px; height:20px"/>					
									</td>
									<td class="thcell" style="padding:12px 5px">
										Название
									</td>
								</tr>
							</table>
						</div>
						<div id="tablecontent" style="margin-top:4px">
							<table class="viewtable" id="viewtable" width="100%">
								<xsl:choose>
