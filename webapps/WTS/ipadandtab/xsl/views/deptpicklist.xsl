...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="entry" mode="item">
		...
		<input type="checkbox" name="chbox" value="{@viewtext}" id="{@docid}" style="width:22px ; height:22px">
			<xsl:if test="userid =''">
				<xsl:attribute name="disabled">disabled</xsl:attribute>
			</xsl:if>
		</input>	
		<font class="font"  style="font-family:verdana; font-size:15px; margin-left:8px; vertical-align:4px">
