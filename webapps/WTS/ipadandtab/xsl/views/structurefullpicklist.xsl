...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="*" mode="line">
		<xsl:if test="name(.) != 'userid'">	
			<div class="Node" style="height:100%; padding:3px 5px;">
	...
	<xsl:template match="entry" mode="item">
		<xsl:if test="./descendant::entry">
			<a href="" style="vertical-align:top; margin-left:3px" id="treedep{@docid}{@doctype}">
				...
				<img border='0' src="/SharedResources/img/classic/minus.gif" id="treedepimg{@docid}{@doctype}" style="width:15px; height:15px"/>
			</a>
		</xsl:if>
		<input type="checkbox" name="chbox" class="chbox"  style="width:22px ; height:22px; margin-left:10px">
		...
		<font href="" style="font-style:arial; font-size:15px; margin-left:8px">
			<xsl:if test="userid != ''">
		...
		<div style="width:100%; text-align:left">
			<xsl:for-each select="query/entry">
				...
				<a href="" style="vertical-align:1px; margin-left:7px" id="treeorg{@doctype}{@docid}">
					...
					<img border='0' src="/SharedResources/img/classic/minus.gif" id="treeorgimg{@doctype}{@docid}" style="width:15px; height:15px"/>
				</a>
				<input type="checkbox" name="chbox" id="" style="width:22px; height:22px; margin-left:10px">
<!-- 					<xsl:attribute name="id" select="@docid"/> -->
				...
				<font style="font-family: Verdana,Arial,Helvetica,sans-serif; font-size:15px; margin-left:8px; vertical-align:4px">
					<xsl:value-of select="replace(@viewtext,'&amp;quot;','&#34;')"/>	
				</font>
