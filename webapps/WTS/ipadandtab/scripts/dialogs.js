...
function keyDown(el){
	if(event.keyCode == 27){
		$("#"+el).css("display","none").empty();
		$("#blockWindow , #"+el).remove();
	}
	if(event.keyCode==13){
...
function pickListSingleOk(docid){
	...
	if (queryOpt.fieldname == "responsible"){
		...
		$("#frm").append("<input type='hidden' name='respost'  id='respost' value='"+$(".post"+docid).val()+"'>")
	}
...
function addSupervisor(query,isMultiValue, field, form, table) {
	enableblockform();
	queryOpt.fieldname = field;
	queryOpt.formname = form;
	queryOpt.isMultiValue = isMultiValue;
	...
	divhtml +="<div id='contentpane' class='contentpane'>Загрузка данных...</div>";  
	divhtml += "<div id='btnpane' class='button_panel' style='margin-top:8%; margin:15px 15px 0; padding-bottom:15px;text-align:right;'>";
	divhtml += "<button onclick='addSupervisorBtnOk()' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' style='margin-right:5px'><span class='ui-button-text'><font style='font-size:12px; vertical-align:top'>ОК</font></span></button>" 
	divhtml += "<button class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onclick='pickListClose()'><span class='ui-button-text'><font style='font-size:12px; vertical-align:top'>"+cancelcaption+"</font></span></button>";    
	divhtml += "</div><div id='executorsColl' display='none'></div></div>";
	$("body").append(divhtml);
	$("#picklist").draggable({handle:"div.header"});
	...
	$.ajax({
		...
		success:function (data){
			if (data.match("login") && data.match("password")){
				...
				$("input[name="+queryOpt.fieldname+"]").each(function(){
...
function addSupervisorSingleOk(docid){
	$("#"+docid).attr("value")
	text=$("#"+docid).attr("value");
	newtr="<tr><td style='width:500px;' class='td_editable'>"+ text +"<input type='hidden' name='"+ queryOpt.fieldname +"'  id='"+queryOpt.fieldname+"' value='"+docid+"'><img style='width:15px; height:15px; margin-right:3px; margin-top:1px; float:right; cursor:pointer' src='/SharedResources/img/iconset/cross.png' onclick='removeTableElement(this)'></td></tr>"
