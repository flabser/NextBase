		...
		success:function (xml){ 
			...
			$(xml).find('response').each(function(){
				...
				if (st =="error" || st =="undefined"){
					if (msgtext.length == 0){
						var msgtext = "Ошибка сохранения"
						if ($.cookie("lang")=="KAZ"){ 
							msgtext = "Сақталу қателігі"
						}else if ($.cookie("lang")=="ENG"){
							msgtext = "Error saving"
						}
						$("#notifydiv").html("<font>"+msgtext+"</font>")
						$("body").hidenotify({"delay":800,"onclose":function(){$("#notifydiv").remove()}})
					}
					if (msgtext.length == 0 && ($.cookie("lang")=="RUS" || !$.cookie("lang"))){ }
					$("#dialog-message").dialog({ 
						buttons: { 
							Ok: function() {
...
function uploadComplete(tableID, doc) {
	...
	var fileName= $('input[name=fname]')[0].files[0].name;
	if (st = 'ok'){
		tableid='#'+tableID;
		var table = $(tableid);
		...
		if(fileName.indexOf(".") != -1){
			...
			$(table).append("<tr id='"+ csf + "'>" +
					...
					"<td>" +
					"<img class='newimageatt' onerror='javascript:changeAttIcon(this)' onload='$(this).removeClass()' src='/SharedResources/img/iconset/file_extension_"+ ext +".png' style='margin-right:5px'><a href='Provider?type=getattach&formsesid="+ sesid+"&field=rtfcontent&file="+fileNameEcr+"' style='vertical-align:6px'>"+ fileName +"</a>&#xA0;&#xA0;" +
...
function markRead(doctype, docid){
	$.ajax({
		type: "GET",
		url: "Provider?type=service&operation=mark_as_read&doctype="+doctype+"&key="+docid,
		cache:false,
		success:function (xml){
...
function AddFields(formname, parent_doctype, parent_docid, newdivid){	
	...
	if(formname == 'animals' || formname == 'farmSownArea'){
		...
		$.ajax({
			...
			datatype:"html",
			success: function(data){
				$("#"+newdivid).append("<form action='Provider' enctype='application/x-www-form-urlencoded' method ='post' name='frm" + fieldCounter+"' id='frm" +fieldCounter+"'></form>");
				$("#frm" + fieldCounter).append(data);					
...
function changeCtrlDate(priority, complication, ctrldate){
...
function changeBriefcontentDemand(docid,doctype){
	$("body").css("cursor","wait")
	if ($.cookie("lang") == "RUS" || !$.cookie("lang")){
		divhtml = "<div id='dialog-revoke' title='Изменить текст содержания'>";
		divhtml += "<div style='height:400px; width:98%; font-size:14px;'>";
		divhtml += "Содержание:<div style='height:10px'/>"; 
	}
	else if($.cookie("lang")=="ENG"){
		divhtml = "<div id='dialog-revoke' title='Change content to demand'>";
		divhtml += "<div style='height:400px;width:98%; font-size:14px;'>";
		divhtml += "Content:"; 
		}
	else if ($.cookie("lang")=="KAZ"){
		divhtml = "<div id='dialog-revoke' title='Мазмұнын өзгерту'>";
		divhtml += "<div style='height:400px;width:98%; font-size:14px;'>";
		divhtml += "Содержание:"; //добавить перевод
		}
...
function revokeDemand(docid, doctype){	 
	$("body").css("cursor","wait")
	var hl = "revokeDemand";
	var title = "Отменить заявку";
	var reason_caption = "Причина"; 
	if($.cookie("lang")=="ENG"){
		title = "Cancel demand";
	...
	$.ajax({
		...
		success: function (msg){
			if ($(msg).find("response").attr("status") == "ok"){
				...
				redirect = $(msg).find("redirect").text();
				setTimeout(function() { $("body").hidenotify({"delay":200,"onclose":function(){ if (redirect == ''){window.history.back()}else{window.location = redirect;}}})},250);
			}
		},
		error: function(data,status,xhr) {
			$('body').css('cursor','default');	
