...
function addDocToFav(el,docid,doctype){
	$.ajax({
		...
		datatype:"XML",
		url: "Provider",
		data: "type=service&operation=add_to_favourites&key="+docid+"&doctype="+doctype+"&dbid=Avanti",
		cache:false,
...
function removeDocFromFav(el,docid,doctype){
	$.ajax({
		...
		datatype:"XML",
		url: "Provider",
		data: "type=service&operation=remove_from_favourites&key="+docid+"&doctype="+doctype+"&dbid=Avanti",
		cache:false,
		success: function (msg){
...
function openParentDocView(element) {
	...
	var doctype = $(element).attr("doctype")
