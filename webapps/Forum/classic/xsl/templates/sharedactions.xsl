...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="save">
		<xsl:if test="document/actionbar/action[@id='save_and_close']/@mode = 'ON'">
			<button style="margin-right:5px; margin-bottom:5px" title="{document/actionbar/action [@id='save_and_close']/@hint}" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="btnsavedoc" autocomplete="off">
				<xsl:attribute name="onclick">javascript:SaveFormJquery('frm','frm','<xsl:value-of select="document/actionbar/action[@id ='close']/js"/>')</xsl:attribute>
				<span>
	...
	<xsl:template name="saveKR">
		<xsl:if test="document/actions/action [.='SAVE']/@enable = 'true'">
			<button style="margin-right:5px" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="{document/actions/action [.='SAVE']/@hint}" id="btnsavedoc" autocomplete="off">
	...
	<xsl:template name="filling_report">
		<xsl:if test="document/actions/action [.='SAVE']/@enable = 'true'">
			<button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="{document/actions/action [.='SAVE']/@hint}" autocomplete="off">
	...
	<xsl:template name="saveKI">
		<xsl:if test="document/actions/action [.='SAVE']/@enable = 'true'">
			<button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="{document/actions/action [.='SAVE']/@hint}" id="btnsavedoc" type="button" autocomplete="off">
