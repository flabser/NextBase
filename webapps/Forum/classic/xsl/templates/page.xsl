...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="outline-section-state">
		<script>
			if($.cookie("FORUM_<xsl:value-of select='@id'/>") != 'null'){
				$("#<xsl:value-of select='@id'/>").css("display",$.cookie("FORUM_<xsl:value-of select='@id'/>"))
				if($.cookie("FORUM_<xsl:value-of select='@id'/>") == "none"){
	...
	<xsl:template name="header-view">	
		<div style="position:absolute; top:0px; left:0px; width:100%; height:70px; border-bottom:1px solid rgba(190,213,224,0.89); background:url('classic/img/blue_background.jpg');">
			...
			<span style="float:left; margin:15px 0 0 0px">
                <font style="font-size:1.6em; color:#404040; padding-left: 10px"><xsl:value-of select="//captions/app_title/@caption" /></font>
                <font style="font-size:0.9em; color:#595959;  padding-left: 10px;"><xsl:value-of select="//captions/app_short_title/@caption" /></font>
			...
			<span style="float:right; padding:5px 5px 5px 0px" >
				...
				<a href="Logout" id="logout" style="text-decoration:none; color:#555555; font:11px Tahoma; font-weight:bold; margin-left:15px">
